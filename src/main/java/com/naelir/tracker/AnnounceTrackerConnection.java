 ...
 *
 * <p>
 * Each call to {@link #buildNextRequest()} advances an internal cursor and
...
public class AnnounceTrackerConnection extends TrackerConnection {
     ...
     *
     * @return the encoded announce request, or an empty array if there are no
     *         hashes to announce
     */
    @Override
    protected byte[] buildNextRequest() {
        if (this.allHashes.isEmpty())
            return new byte[0];
        this.currentHash = this.allHashes.remove(0);
        Sample sample = this.samples.get(this.currentHash);
        if (sample == null) {
            TrackerUdpManager.logger.error("{}, all {}", this.samples.keySet(), this.allHashes);
            return new byte[0];
        }
        sample.check();
    ...
    protected boolean hasMoreBatches() {
        return this.allHashes.isEmpty() == false;
    }
